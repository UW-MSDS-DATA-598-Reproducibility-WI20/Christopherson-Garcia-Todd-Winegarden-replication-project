...
<!--With the R session properly configured, correct packages installed, and data loaded and shaped according to the paper and provided R file, we were then able to proceed with --> We focused out our replication on correlation matrix of lance dimensions shown in Figure 8 of the original paper. The R code used to create the figure was not included in the R file provided by the authors with their publication. Instead, the authors provided a series of five `corr()` statements (shown below) comparing a subset of lance features were included with the resulting correlation coefficients included in the comments.
...
The pairwise correlation coefficients noted in the comments in the code provided by the authors were not the same as the ones shown in their Figure 8 of their published paper. This inconsistency made it more difficult to confirm the success of our replication attempt. 

We were able to replicate the finding that there was found to be an association between overall centroid size and shape that could not be explained through site difference via an ANOVA analysis on page 44 of the original paper

### Reproducing Figure 8 and associated analysis

The figure that we are attempting to replicate is Figure 8, captioned as a  "Correlation matrix of Havor lance dimensions showing Pearson's r(lowerpanel), accompanying bivariate plots (upperpanel) and  distributions (histograms with kernel density estimation, on the diagonal). Red=Ejsbøl, green=Nydam, blue=Illerup."
...
The correlation values do not exactly match that of the paper. A test with the stats package verifies the results of our plot and confirms that the issue is not a mis-labelling of a Spearman coefficient.
...
As of geomorph version 3.1, the function procD.allmoetry has been deprecated (see https://rdrr.io/cran/geomorph/man/procD.allometry.html). This function underpinned the ANOVA tests provided in the technical code appendix. The specificity of the authors' instructions allowed a previous version of geomorph to be installed for this replication. 
